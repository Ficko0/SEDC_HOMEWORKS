...
JOIN
playlists_songs ON playlists_songs.playlist_id = p.id
JOIN
song s ON s.id = playlists_songs.song_id
GROUP BY p.title;

SELECT * FROM playlists_duration pd
WHERE pd.total_duration > '12min';


-- Create a view to display the number of songs in each genre --
CREATE VIEW genre_songs_count AS
SELECT g.name, COUNT(s.id) AS songs_count FROM song s
JOIN
